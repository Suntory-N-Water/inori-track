...
| 項目 | 決定 | 理由 |
|---|---|---|
| 実施スコープ | 全Step実施（Step 1〜4） | MVP（Step 2→4）ではなく全て実施する |
| 既存共有URLの破壊 | 破壊的変更で許容 | シンプルに切り替える |
| テーブル列ヘッダー略称 | 今回スコープ外 | テーブル画面のみの影響。`columns.tsx` のハードコードは維持 |
| スプシ(GAS) | 廃止 | GASは別管理（このリポジトリ外）。スプシはTSファイルと同じフラット構造で、GASはそれをそのまま出力しているだけ。リファクタ後はTSファイルを直接編集する運用に切り替える |
| 曲スラッグ | 計画書のマッピング表を採用 | 変更が必要な場合は一括で後から対応 |
